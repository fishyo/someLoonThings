...
// 只有在包含签到相关请求时才保存
const isSignRequest = $request.url.includes('user-sign') || $request.url.includes('sign');
...
if (authorization && isSignRequest) {
  ...
  if (oldData !== cookieData) {
    ...
    if (saveResult) {
      ...
      $notification.post(cookieName, '❌ Cookie保存失败', '请检查Loon持久化存储权限');
      console.log(`${cookieName} Cookie保存失败`);
    }
  } else {
    console.log(`${cookieName} Cookie未变化，无需更新`);
    $notification.post(cookieName, 'ℹ️ Cookie未变化', '当前Cookie与已保存的相同');
  }
} else {
  let reason = '';
  if (!authorization) {
